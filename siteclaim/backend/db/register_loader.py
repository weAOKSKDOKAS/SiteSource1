...
public-register records (the enforcement-flagged firms) onto their matching register
row by company name — so a curated firm keeps its ``firm_id``, flags and awards while
gaining the register's e-mail/BR/address.

Curated real-provenance firms that are not on the register (e.g. an enforcement-flagged
firm that isn't a registered subcontractor) are kept as their own ``public_register``
rows — the risk overlay. Plain curated rows with no flags are dropped, superseded by the
real register. The illustrative demo firms (``F-*``) carry their own closeout/EOS layer
and are seeded separately via :func:`illustrative_firm` — in SiteSource they, not the
public-register rows, are the assessable/closeout subset the per-tender shortlist draws
from.
"""
...
# Real-provenance curated firms that must survive as an overlay row even without an
# enforcement flag and without a register-name match. SiteSource's demo scenarios run off
# the illustrative ``F-*`` firms (seeded via :func:`illustrative_firm`, provenance
# ``illustrative``), so no *real*-provenance firm needs this protection today — the register
# is authoritative and a plain unflagged curated row is superseded by it. Kept as the
# extension point the competition build used for its drainage-demo firms.
DEMO_FIRM_IDS: frozenset[str] = frozenset()
...
def _canonical_for(group: str, specialty: str) -> set[str]:
    ...
    The specialty keys (field_testing/field_installations/geophysical_survey/
    drainage_works/geotechnical_works) are register-derived groupings beyond the core
    taxonomy; they are produced directly here and never crash on an unmapped specialty —
    the final fallback routes an unknown group/specialty through the taxonomy normaliser
    and then to a general-civil default."""
...
def _curated_firm(c: dict, eos: dict, provenance: str) -> dict:
    """A curated record as its own row (not on the register): the enforcement overlay
    and the illustrative rows (which carry their own closeout/EOS layer)."""
    return {
        "firm_id": c["firm_id"],
...
def _merge(c: dict, row: dict, eos: dict) -> dict:
    ...
    take the register's e-mail/BR/address/dates/registered-trades, union trades."""
...
def merge_register(register: list[dict], curated_real: list[dict], eos_by_id: dict[str, dict]) -> list[dict]:
    ...
    for c in curated_real:
        ...
        # else: plain curated row with no flag — superseded by the register, dropped
    for r in register:
...
def illustrative_firm(rec: dict, eos: dict) -> dict:
    """An illustrative record (the ``F-*`` demo firms) — kept with its closeout/EOS layer
    as provenance ``illustrative`` (demo profile only; never counted in coverage)."""
